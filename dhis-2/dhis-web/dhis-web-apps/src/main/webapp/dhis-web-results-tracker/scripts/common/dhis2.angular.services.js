...
.service('CommonUtils', function($q, $translate, $filter, SessionStorageService, DateUtils, OptionSetService, CurrentSelection, FileService, DialogService, OrgUnitService){
    ...
    return {
        ...
        getOptionComboIdFromOptionNames: function(categoryCombo, options){
            var _ops = options.map(function(op){return op.id;});
            var selectedAttributeOptionCombo = null;
            if( categoryCombo && categoryCombo.categoryOptionCombos && categoryCombo.categoryOptionCombos.length > 0 && options && options.length > 0 ){
                for( var j=0; j< categoryCombo.categoryOptionCombos.length; j++){
                    var ops = categoryCombo.categoryOptionCombos[j].categoryOptions;
                    ops = ops.map(function(op){return op.id;});
                    if ( ops && ops.length &&
                            _ops && _ops.length &&
                            ops.length === _ops.length ){
                        ops = ops.sort();
                        var match = true;
                        for( var i=0; i<_ops.length; i++ ){
                            if( ops.indexOf(_ops[i]) === -1 ){
                                match = false;
                                break;
                            }
                        }
                        ...
                        if( match ){;
                            selectedAttributeOptionCombo = categoryCombo.categoryOptionCombos[j];
                            break;
                        }
                    }
                }
